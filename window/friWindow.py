...
class FriListItem(customWidget):
    ...
    def __init__(self, data, friW):
        ...
        self.findChild(QtWidgets.QLabel, 'account').setText(str(data['id']))
...
class FriWindow(publicWindow):
    ...
    def __init__(self, data):
        ...
        log('connect event func ok')
        # set your info
        self.ctrl['nick'].setText(self.you['nick'])
        self.ctrl['id'].setText(str(self.you['id']))
#       self.ctrl.avatar.setAvatar(self.you.avatar)
        log('set your info ok')
        ...
        log('add friListItem widgets ok')
...
if __name__ == "__main__":
    ...
    #     ]
    # })
    #aaa=FriWindow({'ope': 1, 'status': 1, 'you': {'nick': 'user1', 'avatar': 1, 'id': 1}, 'invite': [{'from': 0, 'online': 0, 'id': 3, 'nick': 'user3', 'avatar': 1, 'status': 1}, {'from': 0, 'online': 0, 'id': 5, 'nick': 'user5', 'avatar': 1, 'status': 1}, {'from': 1, 'online': 1, 'id': 2, 'nick': 'user222', 'avatar': 2, 'status': 1}, {'from': 1, 'online': 0, 'id': 4, 'nick': 'user4', 'avatar': 1, 'status': 0}], 'message': [{'id': 3, 'send': [], 'receive': []}, {'id': 5, 'send': [{'data': 'hello,5', 'time': 1539265825.0, 'readed': 1}], 'receive': [{'data': 'hello,1', 'time': 1539265825.0, 'readed': 1}]}, {'id': 2, 'send': [], 'receive': []}]})
    #aaa.show()
    sys.exit(app.exec_())
